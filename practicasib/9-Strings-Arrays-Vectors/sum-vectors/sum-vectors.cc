...
int main(int argc, char* argv[]) {
  ...
  std::cout << "Lista de vectores: " << std::endl;
  for (const auto& value: my_vector) {
    std::cout << value << " ";
  }
  std::cout << std::endl;
  std::cout << "La suma de los componentes es: " << std::endl;
...
/**
 * Calcula la suma de los elementos de los vectores.
 *
 * @param Vector de tipo double. 
 * @return Suma de los elementos que componen el vector.
 */ 
double ReduceSum(std::vector<double> vector_a_sumar) {
  double suma{0};
  for (auto value : vector_a_sumar) {
    suma += value;
  }
    return suma;
}
